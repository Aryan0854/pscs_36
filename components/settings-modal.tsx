...
export function SettingsModal({
  ...
  }, [userSettings, user, open, setTheme])
  ...
  const handleSettingChange = async (key: keyof UserSettings, value: any) => {
      ...
      try {
        ...
        if (!result.success) {
          ...
          toast({
            ...
            description: "Failed to save settings to server. Please try logging in again.",
            variant: "destructive",
          })
        }
      } catch (error) {
        ...
        toast({
          title: "Settings Error",
          description: "Failed to save settings. Please check your connection and login status.",
          variant: "destructive",
        })
      }
    }
    ...
    if (key === "theme" && !user) {
      // Only allow theme change if not logged in (since theme is forced when logged in)
      setTheme(value)
    }
    ...
    toast({
      ...
      description: `${key.charAt(0).toUpperCase() + key.slice(1)} has been updated successfully.`,
    })
  }
